...
description: "Knative의 Build 컴포넌트는 git 같은 SCM에서 소스를 pull 하고 해당 소스로부터 컨테이너 이미지를 생성하여 레지스트리에 Push 하는 프로세스를 가지고 있다.. 이러한 일련의 Knatvie 빌드 프로세스를 간단한 hello 앱을 만들어 그 동작 원리를 확인해 보고자 한다."
---
...
Knative의 _Build 컴포넌트_ 는 git 같은 SCM에서 소스를 pull 하고 해당 소스로부터 컨테이너 이미지를 생성하여 레지스트리에 Push 하는 프로세스를 가지고 있다. 이러한 일련의 Knative Build 프로세스를 간단한 hello 앱을 만들어 이를 통해 그 동작 원리를 확인해 보고자 한다.
...
* **Build** : 하나의 빌드 프로세스는 여러단계의 스텝으로 구성되어 있으며 하나의 스텝에서는 Builder를 지정한다.
* **Builder** : 컨테이너 이미지 형태로 빌드 프로세스 중 하나의 스텝 또는 전체를 담당하게 된다.
* **BuildTemplate** : 재사용 가능하도록 빌드 프로세스의 Template으로 정의한다.
...
* 문자열을 리턴하는 간단한 샘플 어플을 준비하고 Github Repository 에 _Push_ 한다.
...
* Docker Hub (https://hub.docker.com) 에 가입되어 있지 않다면 가입한다.
* _Secret_ 과 _ServiceAccount_ 생성하고 _Secret_에 _DockerHub_ 계정과 비밀번호를 지정한다.
...
* _DockerHub_ 에 새로운 Repository를 아래와 같이 생성한다.  
...
* `source` 스펙을 통해 빌드할 hello-server 앱의 Github URL을 지정해준다.

* 예제 build.yaml은 아래와 같이 2개의 논리적인 step을 가지고 있다.

1. Github 에서 Pull 된  소스를 Docker Build 한다.
1. Build 된 Docker Image를 Docker Hub로 Push 한다.

### build.yaml 적용

* build.yaml을 적용하고 결과를 확인한다.
...
* 정상적으로 빌드가 되었다면 아래와 같이 _빌드 POD_ 가 생성되고 STATUS가 `Completed` 로 보여진다.
...
* 앞서 살펴본 것과 같이 build.yaml 이 정상 실행되었다면 _빌드 POD_ 가 생성된다.
* 만일 빌드 과정 중 오류가 발생하였다면 `kubectl describe` 명령으로 대상 _POD_ 의 initContainer 수행 상태를 확인을 통해 문제를 해결한다.
* 아래는 정상 처리된 _빌드 POD_ 의 Init Containers 로 실제로는 아래와 같이 4개의 step 이 진행된 것을 확인할 수 있다.
...
* 아래 그림에서와 같이 _Docker Hub_ 에서 Build & Push 된 Image 를 확인할 수 있다.
...
* 정상적으로 서비스되고 있는지 확인하기 위해 `port-forward` 하고 curl로 서비스를 호출해 결과를 확인한다.
...
* 요청 처리를 위해서 아래와 같이 hello-server _POD_ 이 새로 생성된다.
* 생성된 _POD_ 은 사용이 없으면 자동으로 제거된다.
...
예제에서는 이해를 쉽게하기 위해 Pure Docker 빌드를 사용하였지만 일반적으로는 bazel, buildkit, buildpacks, Kaniko 의 [Build Template](https://github.com/knative/build-templates) 를 활용한다면 보다 손쉬운 Knative 빌드를 활용할 수 있을것 같다.
